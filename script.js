...
let testert = new Tree(
  new Tree(new Tree(), new Tree(), new Tree()),
  new Tree(
    new Tree(new Tree(), new Tree(new Tree(), new Tree(), new Tree())),
    new Tree(new Tree(), new Tree(new Tree(), new Tree(), new Tree()))
...
function windowSize() {
  ...
  return [
    min(800, Math.floor((window.innerWidth * 3) / 4)),
...
function windowResized() {
  // p5: runs when window is resized
  if (isFullScreen) {
    resizeCanvas(window.innerWidth, window.innerHeight);
  } else {
    resizeCanvas(...windowSize());
...
function physToCanvas(scalings, x, y) {
  return [
    map(x, scalings.minX, scalings.maxX, padding, width - padding),
    map(y, scalings.minY, scalings.maxY, padding, height - padding),
  ];
}

function canvasToPhys(scalings, x, y) {
  return [
    map(x, padding, width - padding, scalings.minX, scalings.maxX),
    map(y, padding, height - padding, scalings.minY, scalings.maxY),
  ];
}
...
function update() {
  ...
  positions.forEach((p, node) => {
    ...
    p.x += p.vx;
    p.y += p.vy;
    if (p.x <= scalings.minX) {
      scalings.minX = p.x;
    }
    if (p.x >= scalings.maxX) {
      scalings.maxX = p.x;
    }
    if (p.y <= scalings.minY) {
      scalings.minY = p.y;
    }
    if (p.y >= scalings.maxY) {
      scalings.maxY = p.y;
    }
  });
